...
//const API_BASE = 'htto://127.0.0.1:4000/api/nlp';
  ...
  try {
    ...
    switch (action) {
      ...
      case 'reset': {
        ...
        const resetMsg = getLocalized({ en: '🔄 Session reset. Let\'s start over.', am: '🔄 ክፍለ ጊዜ ተቀይሯል። እንደገና እንጀምር።' });
        const intro = await stepIntro();
        return {
          text: `${resetMsg}\n${intro.text}`,
          html: `<div>${resetMsg}</div>${intro.html}`,
          isStructured: true
        };
      }
...
export async function processMessage(message, file) {
  try {
    ...
    // ---- 1. Service switching (keyword-based, unchanged) ----
    ...
    // ---- 2. Yes/No gating (must run BEFORE intent detection) ----
    const state = getState();
    ...
    // ---- 3. Intent detection: ONLY help / status / reset ----
    const det = detectIntent(message);
    if (det && (det.intent === 'help' || det.intent === 'status' || det.intent === 'reset')) {
      console.log(`🎯 Intent detected: ${det.intent} (score=${det.score})`);
      return await executeAction(det.intent, null, message);
    }
    ...
    // ---- 5. Save to current field if valid ----
    const currentField = getCurrentField();
    if (currentField) {
